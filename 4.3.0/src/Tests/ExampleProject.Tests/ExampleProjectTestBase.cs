...
using ExampleProject.Migrations.SeedData;
...
{
    public abstract class ExampleProjectTestBase : AbpIntegratedTestBase<ExampleProjectTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected ExampleProjectTestBase()
        {
        ...
        protected void UsingDbContext(Action<ExampleProjectDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ExampleProjectDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ExampleProjectDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ExampleProjectDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ExampleProjectDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ExampleProjectDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ExampleProjectDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ExampleProjectDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ExampleProjectDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ExampleProjectDbContext, Task<T>> func)
        {
            T result;
